...
# 与 Step 1 prepare 保持一致的 dataset name
...
python3 run_eval_openclaw.py import-sessions --run-name "$RUN_NAME" --dataset-name "$DATASET_NAME"
python3 run_eval_openclaw.py upload --run-name "$RUN_NAME" --model "$MODEL_SHORT" --model-full "$MODEL_FULL" --dataset-name "$DATASET_NAME"
python3 run_eval_openclaw.py pack --run-name "$RUN_NAME"
```

`pack` 命令会输出打包路径和 **`gcloud compute scp` 下载命令**，将该命令完整地展示给用户，例如：
