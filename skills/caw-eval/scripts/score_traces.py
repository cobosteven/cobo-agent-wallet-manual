...
评分机制:
    主评分: openclaw 子 agent（`claude --print`）直接评估所有 S1-S3 维度及综合指标，
            无需额外配置，复用 openclaw 当前模型。
    备用评分: 找不到 openclaw 时自动退回启发式规则（关键词匹配），确保流程不中断。
    评分来源标记: caw.scoring_source = 1.0（子 agent）/ 0.0（启发式）
"""
...
def subagent_judge(
    ...
    *,
    ...
    """
    使用 openclaw 子 agent (claude --print) 作为主评分来源，对 S1-S3 各维度及综合指标进行评估。

    优先让 agent 直接读取 session 文件（通过 --allowedTools Read），
    无文件时将对话文本内嵌入 prompt。
    claude CLI 未找到时返回 available=False，由调用方回退到启发式评分。
    """
    claude_bin = shutil.which("claude")
    if not claude_bin:
        return {"available": False, "note": "skipped (claude CLI not found)"}
  ...
  "comment": "..."
    ...
    try:
        ...
        if proc.returncode != 0:
            ...
            return {"available": False, "note": f"claude exited {proc.returncode}: {stderr[:300]}"}
        raw = (proc.stdout or "").strip()
        start = raw.find("{")
        end = raw.rfind("}") + 1
        if start >= 0 and end > start:
            parsed = json.loads(raw[start:end])
            if all(k in parsed for k in ("s1", "s2", "s3", "e2e")):
                parsed["available"] = True
                return parsed
            preview = raw[:120].replace("\n", " ")
            return {"available": False, "note": f"parse error (missing fields): {preview!r}"}
        preview = raw[:120].replace("\n", " ")
        return {"available": False, "note": f"parse error: {preview!r}"}
    except subprocess.TimeoutExpired:
        return {"available": False, "note": "subagent timeout (180s)"}
    except Exception as e:
        return {"available": False, "note": f"subagent error: {e}"}
...
def score_trace_full(
    ...
    流程:
      ...
      2. 提取各阶段相关文本
      3. 尝试子 agent 评分（主评分）；不可用时退回启发式评分
      4. 计算 E2E 加权综合分
    ...
    # 3-4. Try subagent judge first (primary); fall back to heuristics
    subagent = subagent_judge(
        expected_with_msg, item_metadata,
    ...
    if subagent["available"]:
        ...
        s3 = subagent["s3"]
        task_completion = {
            "score": float(subagent["task_completion"]),
            "method": "subagent",
            "tier": "primary",
            "metric": "task_completion",
        }
        diagnostics = {
            "metric": "diagnostics",
            "tier": "tertiary",
            "error_type": subagent.get("error_type", "none"),
            "recovery_logic": float(subagent.get("recovery_logic", 9.0)),
            "hallucination_risk": float(subagent.get("hallucination_risk", 8.5)),
        }
    ...
    if subagent["available"]:
        e2e["recipe_hit_quality"] = round(float(subagent.get("recipe_hit_quality", e2e["recipe_hit_quality"])), 2)
        e2e["pact_intent_match"] = round(float(subagent.get("pact_intent_match", e2e["pact_intent_match"])), 2)
    ...
    if not subagent["available"]:
        result["subagent_note"] = subagent.get("note", "")
...
def session_main() -> None:
    ...
    claude_bin = shutil.which("claude")
    if claude_bin:
        print(f"[INFO] Subagent judge enabled — {claude_bin} (primary scorer)")
    else:
        print("[INFO] claude CLI not found — falling back to heuristic scoring")
