...
> **CRITICAL:** Once you have called `caw onboard --env sandbox` and received a `session_id`, you **MUST** include `--session-id <SESSION_ID>` on **every** subsequent call. Omitting `--session-id` starts a brand-new session, discarding prior progress and TSS prewarm work.
